...
var newCommand = &cobra.Command{
  ...
  RunE: func(c *cobra.Command, args []string) error {
    ...
    }
    {
      ...
      prompt := &survey.Input{
        Message: "请输入版本名称(参考 https://github.com/jiangbo202/hade_x/releases，默认为最新版本)：",
      }
      ...
      if version != "" {
        // 确认版本是否正确
        release, _, err = client.Repositories.GetReleaseByTag(context.Background(), "jiangbo202", "hade_x", version)
        if err != nil || release == nil {
          fmt.Println("版本不存在，创建应用失败，请参考 https://github.com/jiangbo202/hade_x/releases")
          return nil
        }
      }
      if version == "" {
        release, _, err = client.Repositories.GetLatestRelease(context.Background(), "jiangbo202", "hade_x")
    ...
    fmt.Println("hade_x框架版本：", release.GetTagName())
    ...
    for _, fInfo := range fInfos {
      // 找到解压后的文件夹
      if fInfo.IsDir() && strings.Contains(fInfo.Name(), "jiangbo202-hade_x-") {
    ...
    filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
      ...
      if path == filepath.Join(folder, "go.mod") {
        ...
        c = bytes.ReplaceAll(c, []byte("module github.com/jiangbo202/hade_x"), []byte("module "+mod))
        c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/jiangbo202/hade_x "+version))
      ...
      isContain := bytes.Contains(c, []byte("github.com/jiangbo202/hade_x/app"))
      if isContain {
        fmt.Println("更新文件:" + path)
        c = bytes.ReplaceAll(c, []byte("github.com/jiangbo202/hade_x/app"), []byte(mod+"/app"))
        err = ioutil.WriteFile(path, c, 0644)
        if err != nil {
          return err
        }
